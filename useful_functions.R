...
takeDataReturnAUC_ICGC<- function(frameTrain, frameTest, absentContig){
  ...
  aucTCGA <- round(glmFit$results$ROC, 3)
  ...
  return(list(aucTCGA, rocAUC, glmFit$finalModel$coefficients))
}

################
## F) Function to plot box plot
################
boxPlot <- function(dataPlot, signature, nameProbe = signature, modeLevel, dir.name, status){
  ...
  names(dataPlot)[3:length(names(dataPlot))] <-nameProbe
  ...
  for(i in 1: length(nameProbe)){
    ...
    boxplots_frame<-rbind(boxplots_frame,data.frame(probes=signature[i],mean_cond1 = meanHR,mean_cond2 = meanLR,log2FC=log2FC,wilcoxon_pvalue=wilcoxon_pvalue)) 
    
  }
  
  # The order of contig based on log2FC decreasing
  ctgLogFC <- signature[order(abs(boxplots_frame$log2FC), decreasing = TRUE)]
...
################
## J) Function to plot venne diagram
################
vennePlot <- function(setLR, setNB, setSDC, setES, mode){
  myCol <- brewer.pal(4, "Pastel2")
  
  if(mode=="Gene_level"){
    titleChart = "Gene level"
  }
  if(mode=="Contig_level"){
    titleChart = "Kmer level"
  }
  if(mode=="Combine_Gene_Contig"){
    titleChart = "Combine level"
  }
  
  if(length(setLR) == 1000)
  {
    fileName <- "top1k"
  }else{ fileName <- "signature"}
  # Chart
  chart <- venn.diagram(
    x = list(setLR, setNB, setSDC, setES),
    category.names = c("LR" , "NB " , "FCMedian", "ES"),
    filename = paste0("Result/", mode, "/venn_diagramm_",fileName,".png"),
    output=TRUE,
...
################
## K) Function to take gene name from gene ensembl
################
...
################
## L) Function to calculate performance of signature in specific condition
################
takeDataReturnAUC_cross<- function(frame, NUM_RUNS){
  ...
  calculateAUC <- function(i){
    ...
    data.train <- as.data.frame(frame[ msk,])
    
    data.test <- as.data.frame(frame[!msk,])
  ...
  clusterExport(cl,c('calculateAUC','frame', 'NUM_RUNS','sample.split','trainControl','train','predict','auc','evalmod'),envir=environment())
  
  res <- parLapply(cl, 1:NUM_RUNS, calculateAUC)
  
  auc <- do.call("rbind", lapply(res, "[[", 1))
  meanAUC <- round(mean(auc),3)
  sdAUC <- round(sd(auc),3)
  
  return(c(meanAUC,sdAUC))
  stopCluster(cl)
}