...
takeDataReturnAUC_ICGC<- function(frameTrain, frameTest, absentContig){
  ...
  return(list(aucTCGA, rocAUC, varImp(glmFit)))
}
...
boxPlot <- function(dataPlot, modeLevel, dir.name, status){
  ...
  for(i in 1: length(nameProbe)){
    ...
    boxplots_frame<-rbind(boxplots_frame,data.frame(probes=nameProbe[i],mean_cond1 = meanHR,mean_cond2 = meanLR,log2FC=log2FC,wilcoxon_pvalue=wilcoxon_pvalue)) 
...
################
## H) Function to calculate performance of signature in specific condition
################
takeDataReturnAUC_cross<- function(frame, NUM_RUNS){
  # Calculate mean and sd for signature
  #
  # Args:
  #   frame: Data frame containes selected probes
