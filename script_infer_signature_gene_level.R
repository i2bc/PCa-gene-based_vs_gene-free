...
dir.store <- paste0("Result_artical/gene_level")
...
pipeline <- function(topProbesPath, samplesConditionDisPath, dataValidPath, samplesConditionValidPath, numruns){
  ...
  print(paste("Performance of gene signature in TCGA:", resSign[[1]]))
  print(paste("Performance of gene signature in ICGC:", resSign[[2]]))
  ...
  print(paste("Performance of gene signature in TCGA (cross validation):", resSignCross[[1]], "+/-", resSignCross[[2]]))
  ...
  return (list(signature.save, resSignCross, dataSaveTCGA, dataSaveICGC))
