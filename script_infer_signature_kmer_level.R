...
pipeline <- function(topProbesPath, samplesConditionDisPath, dataValidPath, samplesConditionValidPath, numruns){
  ...
  print("List contig Name in TCGA")
  print(names(dataSigTCGA))
  print("List contig Name in ICGC")
  print(names(dataSigICGC))
  ...
  print(dim(dataSigICGC))

  resSign <- takeDataReturnAUC_ICGC(frameTrain = dataSigTCGA, frameTest = dataSigICGC, absentContig = absentCtg)
  
  name.file <- paste0(dir.store, "/importance-sig-contig.tsv")
  
  print(paste("Performance of contig in TCGA:", resSign[[1]]))
  print(paste("Performance of contig in ICGC:", resSign[[2]]))
  
  # Store important variable 
  name.file <- paste0(dir.store, "/import-sig-contig.tsv")
  importVar <- as.data.frame(resSign[[3]]$importance)
  importVar$Gene <- rownames(resSign[[3]]$importance)
  ...
  print(paste("Performance of contig in TCGA (cross validation):", resSignCross[[1]], "+/-", resSignCross[[2]]))
